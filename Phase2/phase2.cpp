...
private:
    ...
    void READ()
    {
        ...
        int i=0, memorty_ptr=RA, end=RA+10;
        while(i<40 && buffer[i]!='\0' && memorty_ptr<end){
            for(int i=0; i<4; i++){
                Memory[memorty_ptr][i]=buffer[i];
                i++;
            }
            memorty_ptr++;
        }
    ...
    { 
        ...
        {   
...

            {
                SIMULATION();
                page_fault_valid = 1;
                if ((isdigit(IR[2])==false) || (isdigit(IR[3])== false))
                {
                    PI = 2;
                    MOS();
                }
